package com.example.gymtime.ui.theme

import androidx.compose.ui.graphics.Color

// Primary Color Palette - Modern Lime Green Theme
val PrimaryAccent = Color(0xFFA3E635) // Lime Green
val PrimaryAccentDark = Color(0xFF84CC16) // Darker Lime
val PrimaryAccentLight = Color(0xFFBEF264) // Lighter Lime

// Background & Surface Colors (Dark Mode defaults)
val BackgroundCanvas = Color(0xFF121212) // Almost Black
val SurfaceCards = Color(0xFF0D0D0D) // Very dark, almost black
val GradientStart = Color(0xFF0A1A0A) // Dark green tint for gradient
val GradientEnd = Color(0xFF0A0A0A) // Very dark black

// Text Colors (Dark Mode defaults)
val TextPrimary = Color(0xFFFFFFFF) // Near White
val TextSecondary = Color(0xFFE0E0E0) // Near White
val TextTertiary = Color(0xFF9CA3AF) // Muted gray

// Light Mode Colors
val LightBackgroundCanvas = Color(0xFFF5F5F5)
val LightSurfaceCards = Color(0xFFFFFFFF)
val LightTextPrimary = Color(0xFF1A1A1A)
val LightTextSecondary = Color(0xFF4A4A4A)
val LightTextTertiary = Color(0xFF7A7A7A)
val LightInputBackground = Color(0xFFEEEEEE)
val DarkInputBackground = Color(0xFF1A1A1A)
val ElevatedCardSurface = Color(0xFF161616)
val DialogSurface = Color(0xFF222222)

// Status Colors
val SuccessFresh = Color(0xFF2ECC71) // Emerald Green
val WarningFatigued = Color(0xFFE74C3C) // Alizarin Red
val RecoveryFresh = PrimaryAccent
val RecoveryRecovering = Color(0xFFF59E0B)
val RecoveryFatigued = Color(0xFFEF4444)
val StreakResting = Color(0xFF64B5F6)
